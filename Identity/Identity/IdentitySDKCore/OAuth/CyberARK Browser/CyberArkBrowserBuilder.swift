...
public class CyberArkBrowserBuilder: NSObject {
    ...
    init(_ oAuthEndPoint: OAuthEndPoint? = nil, pkce: AuthOPKCE? = nil) {
        self.oAuthEndPoint = oAuthEndPoint ?? OAuthEndPoint()
        self.pkce = pkce
    }
    ...
    @objc public func build() -> CyberArkBrowser {
        let browser = CyberArkBrowser(type: self.webType, presentingViewController: self.presentingViewController,oauthClient: self.oAuthEndPoint, pkce: self.pkce)
        return browser
    }
