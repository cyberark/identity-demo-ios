...
extension OAuthEndPoint {
    ...
    func getAuthenticationEndpoint(code: String) -> Endpoint {
        ...
        let postData = NSMutableData(data: "\(OAuth2Header.grantType.rawValue)=\(OAuth2Header.code.rawValue)".data(using: String.Encoding.utf8)!)
        ...
        postData.append("&\(OAuth2Header.codeChallenge.rawValue)=\(self.pkce?.challenge ?? "")".data(using: String.Encoding.utf8)!)
